...
class Mask:
    ...
    def img_preprocessing(self, img):
        ...
        height, width = img.shape[:2]
        def grad_filtering(x_grad, y_grad, mag_thresh, ori_thresh):
        ...
        gray_filtered_mask = grad_filtering(x_grad_gray, y_grad_gray, self.mag_thresh, self.ori_thresh)
        Cr_filtered_mask = grad_filtering(x_grad_Cr, y_grad_Cr, self.mag_thresh, self.ori_thresh)
        s_filtered_mask = grad_filtering(x_grad_s, y_grad_s, self.mag_thresh, self.ori_thresh)
...
#            pts = [tmp_center_pts_left_x, tmp_center_pts_right_x, tmp_center_pts_y]
    ...
    def find_lane(self, warped_grad_mask, warped_color_mask):
        ...
        new_left_windows_centers = []
        new_right_windows_centers = []
        ...
        for i in range(self.n_windows):
            ...
            # redefine the windows center points with average x-coordinate of line-candidate-points
            if len(tmp_x_left_cand) > 20:
                new_left_windows_centers.append([np.mean(tmp_x_left_cand).astype(np.int32), left_windows_centers[i][1]])
            else:
                new_left_windows_centers.append(list(left_windows_centers[i]))

            if len(tmp_x_right_cand) > 20:
                new_right_windows_centers.append([np.mean(tmp_x_right_cand).astype(np.int32), right_windows_centers[i][1]])
            else:
                new_right_windows_centers.append(list(right_windows_centers[i]))
        ...
        if (len(x_pts_left) < self.detected_pts_limit):
            ...
            self.left_windows_centers = np.array(new_left_windows_centers, dtype = np.int32)
        ...
        if (len(x_pts_right) < self.detected_pts_limit):
            ...
            self.right_windows_centers = np.array(new_right_windows_centers, dtype = np.int32)
